...
*/
package org.onosproject.linkmeasurement.rest;
...
/** Mao Service */
import org.onosproject.linkmeasurement.intf.MaoLinkQualityService;
...
public class MaoRestResource extends AbstractWebResource {
    ...
    private MaoLinkQualityService maoLinkQualityService = get(MaoLinkQualityService.class);
    ...
    @GET
    @Path("getLinksLatency")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getLinksLantency() {
        MaoLinkQualityService service = getService(MaoLinkQualityService.class);
        
        ObjectNode root = mapper().createObjectNode();
        ArrayNode arrayNode = root.putArray("LinksLanency");

        // maoLinkQualityService.getAllLinkLatencies().forEach((k, v) -> print("%s --- %dms", k.toString(), v));
        maoLinkQualityService.getAllLinkLatencies().forEach((link, latency) -> {
            
            ObjectNode node = mapper().createObjectNode();
            
            node.put("link", link.toString());
            node.put("latency", latency);
    ...
    @GET
    @Path("getLinksLatencyController")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getLinksLatencyController() {
        MaoLinkQualityService service = getService(MaoLinkQualityService.class);
        ...
        // maoLinkQualityService.getAllLinkLatencies().forEach((k, v) -> print("%s --- %dms", k.toString(), v));
        maoLinkQualityService.getAllControlLatencies().forEach((device, latency) -> {
            
            ObjectNode node = mapper().createObjectNode();
            
            node.put("device", device.toString());
            node.put("latency", latency);
